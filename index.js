//Aqui se establecen los select dinamicos
...
for(var i = 0; i < 24; i++){
    STATES.push(selectPrincipal.options[i].value)
}

const municipality = () => {
    while(selectMunicipio.length != 0){
        deletedOption = selectMunicipio.firstChild
        selectMunicipio.removeChild(deletedOption)
        delete deletedOption
    }

    index = selectPrincipal.selectedIndex
    selectedState = DB[index]
...
const city = () => {
    while(selectCiudad.length != 0){
        deletedOption = selectCiudad.firstChild
        selectCiudad.removeChild(deletedOption)
        delete deletedOption
    }

    let index = selectPrincipal.selectedIndex
    let selectedState = DB[index]
    
    if(selectedState.ciudades){
        for(var i = 0; i < selectedState.ciudades.length; i++){
            option = document.createElement('option')
            option.value = selectedState.ciudades[i]
            option.text = selectedState.ciudades[i]
            selectCiudad.appendChild(option)
        }
    }else{
        for(var i = 0; i < selectedState.municipios.length; i++){
            option = document.createElement('option')
            option.value = selectedState.municipios[i].capital
            option.text = selectedState.municipios[i].capital
            selectCiudad.appendChild(option)
        }
    }
...
const TRIGLICERIDO = document.getElementById('triglicerido')
const ACIDO = document.getElementById('acido')
const SEX = document.getElementById('sex')
const numberInput = [CEDULA, EDAD, PHONE, GLICEMIA, COLESTEROL, TRIGLICERIDO, ACIDO]
...
const numberValidator = (element) => {
    if(isNaN(element) || element < 0 || element.length == 0){
        swal.fire("Error", "Debes ingresar un dato númerico válido", "error")
        return 0
    }else{
        return 1
    }
...
const bringResults = () => {
    var index0 = selectPrincipal.selectedIndex
    var index1 = selectMunicipio.selectedIndex
    var index2 = selectCiudad.selectedIndex
    var index3 = SEX.selectedIndex
    ...
    let results = {
        ...
        sexo: SEX.options[index3].text,
        estado: selectPrincipal.options[index0].text,
        municipio: selectMunicipio.options[index1].text,
        ciudad: selectCiudad.options[index2].text,
        glicemia: '',
        colesterol: '',
        triglicerido: '',
        acido_urico: ''
    ...
    let examsType = numberInput.slice(3, numberInput.length)
    let intervals = [[70, 110], [60, 200], [50, 140], [4.8, 6.8]]

    for(var i = 0; i < examsType.length; i++){
        if(examsType[i].value >= intervals[i][0] && examsType[i].value <= intervals[i][1]){
            results[examsType[i].name] = `Sus valores de ${examsType[i].name} se encuentran estables con registros de ${examsType[i].value}`
        }else if(examsType[i].value < intervals[i][0]){
            results[examsType[i].name] = `sus valores de ${examsType[i].name}, se encuentran bajos, por favor pida una cita`
        }else{
            results[examsType[i].name] = `sus valores de ${examsType[i].name}, se encuentran altos, por favor pida una cita`
        }
    }

    Swal.fire({
        title: '<strong>Resultados del<u> paciente</u></strong>',
        icon: 'info',
        html:
          `<h1> <b>${results.fullname}</b> </h1></br>` +
          `cedula: ${results.cedula} </br>` +
          `edad: ${results.edad} </br>` +
          `telefono: ${results.telefono} </br>` +
          `estado: ${results.estado} </br>` +
          `municipio: ${results.municipio} </br>` +
          `ciudad: ${results.ciudad} </br></br>` +
          `<h3>Sus examenes </h3></br>` +
          `Glicemia: ${results.glicemia} </br>` +
          `Colesterol: ${results.colesterol} </br>` +
          `Triglicérido: ${results.triglicerido} </br>` +
          `Ácido Úrico: ${results.acido_urico} </br>`,
    ...
    console.log(results)
    console.log(examsType)

}

const validator = (fullname) => {
    var textS = textValidator(fullname.value)
    ...
    for(var i = 0; i < numberInput.length; i++){
        ...
        var numberS = numberValidator(numberInput[i].value)
        console.log(numberS)
        if(!numberS){
            return 0
        }
    ...
    if(!(/^\d{10}$/.test(PHONE.value))){
        swal.fire("Error", "Debes ingresar un numero telefonico válido", "error")
        return 0
    }
...
const Executer = () => validator(FULLNAME)

const buttonHandler = () => button.addEventListener('click', Executer)
buttonHandler()
